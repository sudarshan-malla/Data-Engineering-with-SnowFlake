...
weather_hamburg_reference_associations = permission.get_reference_associations("weather_hamburg_app")

if len(weather_hamburg_reference_associations) == 0:
    permission.request_reference("weather_hamburg_app")

# weather_hamburg_view = "reference('weather_hamburg_app')"

st.title('Weather and Sales Trends for Hamburg, Germany')

# Load the view and create a pandas dataframe
hamburg_weather = session.table("weather_hamburg_app").select(
    col("DATE"),
    col("DAILY_SALES"),
